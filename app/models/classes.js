...
module.exports = {
  ...
  getDetailClass: (conn, classId, callback) => {
    conn.getConnection((errConnection, connection) => {
      ...
      connection.query(`SELECT a.classid, a.rating, a.name, a.cover, a.description, a.created_at, b.fullname AS guru, b.profile_picture FROM classes_tab a LEFT JOIN guru_tab b ON a.guruid=b.guruid LEFT JOIN users_classes_tab c ON a.classid=c.classid WHERE a.status=1 AND a.classid=? GROUP BY a.classid`, classId, (err, rows) => {
        callback(err, rows)
      })
    })
  },
