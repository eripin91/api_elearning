...
module.exports = {
  get: (conn, userId, classId, callback) => {
    conn.getConnection((errConnection, connection) => {
      ...
      connection.query('SELECT c.* FROM courses_tab c LEFT JOIN classes_tab ct ON c.classid = ct.classid WHERE c.classid = ? AND c.status = 1', classId, (err, rows) => {
        let data = rows[0]
        ...
        if (data === undefined) {
          ...
          connection.query('SELECT cd.detailid, cd.name, SUM(cm.duration) as durasi, (SELECT b.is_completed FROM courses_detail_tab a LEFT JOIN users_course_detail_tab b ON a.detailid = b.detailid LEFT JOIN users_tab c ON b.userid = c.userid WHERE a.detailid = cd.detailid AND c.userid = ?) as is_completed FROM courses_detail_tab cd LEFT JOIN courses_material_tab cm ON cd.detailid = cm.detailid WHERE cd.courseid = ? AND cd.status = 1 GROUP BY(cd.detailid)', [userId, data.courseid], (err, result) => {
            data.course = result
            callback(err, data)
          })
        }
  ...
  getMaterials: (conn, materialId, callback) => {
    conn.getConnection((errConnection, connection) => {
      if (errConnection) console.log(errConnection)
      connection.query('SELECT duration from courses_material_tab WHERE materialid = ? AND status = 1', [materialId], (err, result) => {
  ...
  getStatusDownload: (conn, userId, materialId, callback) => {
    conn.getConnection((errConnection, connection) => {
      ...
      connection.query('SELECT um.is_downloaded FROM users_material_progress_tab um LEFT JOIN users_tab u ON um.userid = u.userid LEFT JOIN courses_material_tab cm ON um.materialid = cm.materialid WHERE um.userid = ? AND um.materialid = ? AND um.status = 1', [userId, materialId], (rows) => {
        callback(rows)
      })
    })
  },
  ...
  getUserMaterial: (conn, userId, callback) => {
    conn.getConnection((errConnection, connection) => {
      if (errConnection) console.error(errConnection)
      connection.query('SELECT cm.materialid, cm.name, cm.thumbnails, cm.duration, um.is_done_watching, um.is_downloaded FROM users_material_progress_tab um JOIN users_tab u ON um.userid = u.userid JOIN courses_material_tab cm ON cm.materialid = um.materialid where um.userid = ? AND um.is_downloaded = 1 AND um.status = 1', userId, (err, rows) => {
        callback(err, rows)
      })
    })
  },
  getNextMaterial: (conn, data, callback) => {
    conn.getConnection((errConnection, connection) => {
      if (errConnection) console.error(errConnection)
      connection.query('SELECT cm.materialid, cd.detailid, cm.name, cm.thumbnails, cm.duration FROM courses_material_tab cm JOIN courses_detail_tab cd ON cm.detailid = cd.detailid WHERE cm.detailid = ? AND cm.materialid > ? LIMIT 3 AND cm.status = 1', data, (err, rows) => {
        callback(err, rows)
      })
    })
  },
  checkUserCourseDetail: (conn, userId, detailId, callback) => {
    conn.getConnection((errConnection, connection) => {
      if (errConnection) console.error(errConnection)
      connection.query('SELECT * FROM users_course_detail_tab WHERE userid = ? AND detailid = ? AND status = 1', [userId, detailId], (err, rows) => {
        callback(err, rows)
      })
    })
  },
  checkUserMaterial: (conn, materialId, callback) => {
    conn.getConnection((errConnection, connection) => {
      if (errConnection) console.error(errConnection)
      connection.query('SELECT um.*, cm.name as material_name FROM users_material_progress_tab um JOIN courses_material_tab cm ON um.materialid = cm.materialid WHERE um.materialid = ? AND um.status = 1', materialId, (err, rows) => {
        callback(err, rows)
      })
    })
  },
  ...
  checkDetailMaterial: (conn, detailId, callback) => {
    conn.getConnection((errConnection, connection) => {
      if (errConnection) console.error(errConnection)
      connection.query('SELECT * FROM users_course_detail_tab WHERE detailId = ? AND status = 1', detailId, (err, rows) => {
        callback(err, rows)
      })
    })
  },
  ...
  checkUserClass: (conn, classId, callback) => {
    conn.getConnection((errConnection, connection) => {
      if (errConnection) console.error(errConnection)
      connection.query('SELECT * FROM users_classes_tab WHERE classid = ? AND status = 1', classId, (err, rows) => {
        callback(err, rows)
      })
    })
  },
  ...
  checkerClassDone: (conn, classId, userId, callback) => {
    conn.getConnection((errConnection, connection) => {
      if (errConnection) console.error(errConnection)
      connection.query('SELECT uc.*, ct.name FROM users_classes_tab uc LEFT JOIN classes_tab ct ON uc.classid = ct.classid WHERE uc.userid = ? AND uc.classid = ? AND uc.status = 1', [userId, classId], (err, rows) => {
        callback(err, rows)
      })
    })
  },
  getTestCourseDetail: (conn, assessmentId, callback) => {
    conn.getConnection((errConnection, connection) => {
      if (errConnection) console.error(connection)
      connection.query(`SELECT COUNT(IF(question_type = 'single-choice', 1, null)) AS single_choice, COUNT(IF(question_type = 'essay', 1, null)) AS essay FROM assessment_detail_tab WHERE assessmentid = ? AND status = 1`, [assessmentId], (err, rows) => {
        callback(err, rows)
      })
    })
  },
  checkTestCourseDone: (conn, assessmentId, userId, callback) => {
    conn.getConnection((errConnection, connection) => {
      connection.query(`SELECT * FROM users_assessment_tab ua LEFT JOIN assessment_detail_tab ad ON ua.detailassessmentid = ad.detailid WHERE ua.userid = ? AND ad.assessmentid = ? AND ad.status = 1`, [userId, assessmentId], (err, rows) => {
        if (errConnection) console.error(errConnection)
        callback(err, rows)
      })
    })
