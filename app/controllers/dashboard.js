    ...
    (data, cb) => {
      ...
      data.user_classes_list.map((item) => {
        dashboardModel.getUserClassProgress(req, req.params.userId, item.classid, (errClass, resultClass) => {
          err = errClass
          item.user_progress = resultClass[0].progress_bab
          item.total_bab = resultClass[0].all_bab
        })
      })
      cb(err, data)
    },
    ...
    (data, cb) => {
      async.eachSeries(data.recomendation_class, (item, next) => {
        dashboardModel.getDetailCount(req, item.classid, (err, result) => {
          ...
          result.map((total) => {
            item.total_course = total.total_bab
          })
          next()
        })
      }, err => {
      ...
      async.eachSeries(data.recomendation_class, (item, next) => {
        dashboardModel.getDurationCount(req, item.classid, (err, result) => {
          result.map((total) => {
            ...
            item.total_durasi = total.total_durasi
          })
          next()
        })
      }, err => {
